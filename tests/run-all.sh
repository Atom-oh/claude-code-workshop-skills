...
# Note: literal-string containment, not `grep -qP` — GNU grep's `-P` (PCRE) isn't
# available on macOS/BSD grep, and this repo's own hook_match.py docstring warns that a
# failing `grep -qP` there silently no-ops. Keep this test framework itself immune to
# that trap.
assert_grep_match() {
    local desc="$1" pattern="$2" input="$3"
    echo "$input" | python3 -c "import sys,re; sys.exit(0 if re.search(sys.argv[1], sys.stdin.read()) else 1)" "$pattern" \
        && pass "$desc" || fail "$desc" "pattern '$pattern' did not match"
}

assert_grep_no_match() {
    local desc="$1" pattern="$2" input="$3"
    echo "$input" | python3 -c "import sys,re; sys.exit(0 if re.search(sys.argv[1], sys.stdin.read()) else 1)" "$pattern" \
        && fail "$desc" "pattern '$pattern' matched (expected no match)" || pass "$desc"
}
...
echo -e "${CYAN}=== claude-code-workshop-skills Test Suite ===${NC}"
echo ""
